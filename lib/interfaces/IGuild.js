...
class IGuild extends IBase {
   ...
   * @param {Number|auditLogAction|null} [actionType]
   * All options are optional.
   * @returns {Promise<Object, Error>}
   */
  getAuditLogs (userId, actionType, before, limit) {
    ...
    if (typeof actionType === 'string' && Constants.AuditLogActions[actionType]) {
      actionType = Constants.AuditLogActions[actionType]
    } else if (typeof actionType === 'number' && Object.values(Constants.AuditLogActions).includes(actionType)) {
      actionType = actionType
    } else {
      return Promise.reject('actionType is not valid.')
    }
    return rest(this._discordie)
      .guilds.getAuditLogs(this.id, userId, actionType, limit, before)
  }
