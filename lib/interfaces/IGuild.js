 ...
 * Base guild class.
 *
 * Use the `auditLogAction` property to provide a string or number describing
 * the audit log action you want to filter by:
 *
 * ```js
 * Discordie.AuditLogAction: {
 *   GUILD_UPDATE: 1,
 *   CHANNEL_CREATE: 10,
 *   CHANNEL_UPDATE: 11,
 *   CHANNEL_DELETE: 12,
...
class IGuild extends IBase {
  ...
  get iconURL () {
    ...
    return Constants.CDN_ENDPOINT + Endpoints.CDN_GUILD_ICON(this.id, this.icon)
  }
   ...
   * Deprecated for newer guilds as no general channel is set.
   * @returns {ITextChannel|null}
   * @readonly
   */
  get generalChannel () {
  ...
  createRole () {
    return new Promise((rs, rj) => {
      rest(this._discordie)
        .guilds.roles.createRole(this.id)
  ...
  getAuditLogs (userId, auditLogAction, before, limit) {
    ...
    if (typeof auditLogAction === 'string' && Constants.AuditLogActions[auditLogAction]) {
      auditLogAction = Constants.AuditLogActions[auditLogAction]
    } else if (typeof auditLogAction === 'number' && Object.values(Constants.AuditLogActions).includes(auditLogAction)) {
  ...
  getPruneEstimate (days) {
    return rest(this._discordie)
      .guilds.prune.getPrune(this.id, days)
  }
  ...
  pruneMembers (days) {
    ...
    return rest(this._discordie)
      .guilds.prune.postPrune(this.id, days)
  }
  ...
  editWidget (options) {
    return rest(this._discordie)
      .guilds.patchEmbed(this.id, options || {})
  }
  ...
  uploadEmoji (image, name) {
    ...
    return rest(this._discordie)
      .guilds.emoji.postEmoji(this.id, image, name)
  }
  ...
  deleteEmoji (emoji) {
    ...
    return rest(this._discordie)
      .guilds.emoji.deleteEmoji(this.id, emoji)
  }
