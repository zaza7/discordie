...
class IAuthenticatedUser extends IBase {
   ...
   * @param {Object|String|null} [activity] - Object `{name: String}` or string
   * @example
   ...
   * //       only "http://twitch.tv/" and "https://youtube.com/" urls will work
   * client.User.setStatus("idle", activity); // idle, playing
   * client.User.setStatus(null, activity); // no status change, playing
   ...
   * client.User.setStatus("online", activity); // online, playing
  ...
  setStatus(status, activity) {
    ...
    if (activity === undefined) activity = this.activity;
    if (typeof activity === "string") activity = {name: activity};
    ...
    if (this._discordie._user) {
      this._discordie._user = this._discordie._user.merge({status, activity, afk});
    }
